...
function NoticeBox() {
  ...
  const handleDownloadPDF = (pdfFile) => {
    if (!pdfFile) return;
    window.open(`${BASE_URL}/uploads/${pdfFile}`, "_blank");
  };
            ...
            {noticesData?.length > 0 ? (
              noticesData.map((notice, index) => (
                <div key={index} className="py-2 h-fit">
                  ...
                  {notice.pdfFile && (
                    <button className="download-btn text-blue-500 hover:text-blue-700 ml-2" onClick={() => handleDownloadPDF(notice.pdfFile)}>
                      📄 Download PDF
                    </button>
                  )}
                </div>
