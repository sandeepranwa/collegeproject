...
function Signup() {
  document.title = 'BIET | Student Sign Up';
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData?.email || !formData?.first_name || !formData?.number || !formData?.password) {
      ...
      if(!formData?.role) role = 'Role Required!';
      if (!formData?.password) password = 'Password Required!';
      setError({ email: email, first_name: first_name, number: number,role:role, password: password })
      return;
    }
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-center py-2 min-h-screen bg-white ">
